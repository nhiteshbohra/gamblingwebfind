...
def create_batches(csv_path, pdf_path, output_root="Gambling sites", batch_size=40, dpi=96, quality=75, optimize=True):
    """
    Splits a CSV file and a matching PDF file into batch folders of `batch_size`.
    Each batch folder gets a CSV slice with header and a PDF slice with corresponding pages optimized for screen/web quality.
    
    :param csv_path: Path to source CSV
    :param pdf_path: Path to source PDF
    :param output_root: Root folder for batch subfolders
    :param batch_size: Number of items per batch (default: 40)
    :param dpi: Screen export DPI resolution (default: 96 for wdExportOptimizeForOnScreen equivalent)
    :param quality: JPEG compression quality (default: 75)
    :param optimize: Whether to apply screen quality optimization (Lower DPI, smaller file size)
    """
    ...
    for batch_num in range(1, total_batches + 1):
        ...
        for page_idx in range(start_idx, end_idx):
            ...
            if optimize:
                # Render page at screen DPI and insert compressed raster stream
                ...
                # Insert original vector/raster content directly
    ...
    src_doc.close()
    print("\nAll batches created successfully with screen/web quality optimization!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--size", type=int, default=40, help="Batch size (default: 40)")
